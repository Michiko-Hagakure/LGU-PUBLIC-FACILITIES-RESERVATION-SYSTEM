...
document.addEventListener('DOMContentLoaded', function() {
    const sidebar = document.getElementById('cbd-sidebar');
    const sidebarToggle = document.getElementById('sidebar-toggle');
    const sidebarClose = document.getElementById('cbd-sidebar-close');
...
// Settings Dropdown
const settingsButton = document.getElementById('cbd-settings-button');
const settingsDropdown = document.getElementById('cbd-settings-dropdown');
...
function confirmCbdLogout() {
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            title: 'Sign Out?',
            text: "You will be logged out of the CBD portal.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                document.getElementById('cbdLogoutForm').submit();
            }
        });
    } else {
...
window.confirmCbdLogout = confirmCbdLogout;
...
style.textContent = `
    ...
    #cbd-sidebar nav::-webkit-scrollbar {
        width: 4px;
    }
    
    #cbd-sidebar nav::-webkit-scrollbar-track {
        background: transparent;
    }
    
    #cbd-sidebar nav::-webkit-scrollbar-thumb {
        background: #faae2b;
        border-radius: 4px;
    }
    
    #cbd-sidebar nav::-webkit-scrollbar-thumb:hover {
        background: #e09900;
    }
`;
...
function setActiveLink() {
    ...
    sidebarLinks.forEach(link => {
        ...
        if (linkHref.includes('/cbd/reports/revenue') && currentPath.includes('/cbd/reports/revenue')) {
            link.classList.add('active');
            activeFound = true;
        }
        else if (linkHref.includes('/cbd/reports/facility-utilization') && currentPath.includes('/cbd/reports/facility-utilization')) {
            link.classList.add('active');
            activeFound = true;
        }
        else if (linkHref.includes('/cbd/reports/budget-analysis') && currentPath.includes('/cbd/reports/budget-analysis')) {
            link.classList.add('active');
            activeFound = true;
        }
        ...
        else if ((linkHref.includes('/cbd/dashboard') || linkHref.includes('cbd.dashboard')) && currentPath === '/cbd/dashboard') {
            if (!activeFound) {
                link.classList.add('active');
                activeFound = true;
            }
...
<?php /**PATH C:\laragon\www\local-government-unit-1-ph.com\resources\views/components/sidebar/cbd-script.blade.php ENDPATH**/ ?>