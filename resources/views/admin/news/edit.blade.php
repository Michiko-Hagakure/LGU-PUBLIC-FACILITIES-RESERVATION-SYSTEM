...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit News Article</h1>
    ...
    <form action="{{ URL::signedRoute('admin.news.update', $newsItem->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $newsItem->slug) }}" required
            ...
            <div>
                ...
                <select name="category" id="category" required
                    ...
                    <option value="general" {{ old('category', $newsItem->category) == 'general' ? 'selected' : '' }}>General</option>
                    <option value="facility_update" {{ old('category', $newsItem->category) == 'facility_update' ? 'selected' : '' }}>Facility Update</option>
                    <option value="policy_change" {{ old('category', $newsItem->category) == 'policy_change' ? 'selected' : '' }}>Policy Change</option>
                    <option value="maintenance" {{ old('category', $newsItem->category) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                    <option value="emergency" {{ old('category', $newsItem->category) == 'emergency' ? 'selected' : '' }}>Emergency</option>
                </select>
                @error('category')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="hidden" name="published_at" id="published_at" value="{{ old('published_at', $newsItem->published_at ? $newsItem->published_at->format('Y-m-d') : '') }}" required>
                <div id="published_at_display" onclick="openDatePicker()"
                    ...
                    <span id="published_at_text" class="{{ $newsItem->published_at ? 'text-gray-800' : 'text-gray-500' }}">
                        {{ $newsItem->published_at ? $newsItem->published_at->format('l, F j, Y') : 'Select publish date...' }}
                    </span>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="excerpt" id="excerpt" rows="2" maxlength="500"
                    ...
                    placeholder="Brief summary of the article (max 500 characters)">{{ old('excerpt', $newsItem->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="content" id="content" rows="8" required
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('content') border-red-500 @enderror">{{ old('content', $newsItem->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $newsItem->image_path) }}" alt="Current image" class="w-40 h-24 object-cover rounded-lg border">
            ...
            <div class="md:col-span-2 flex flex-wrap gap-6">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $newsItem->is_featured) ? 'checked' : '' }}
                        class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="ml-2 text-sm text-gray-700">Featured Article</span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_urgent" value="1" {{ old('is_urgent', $newsItem->is_urgent) ? 'checked' : '' }}
                        class="w-5 h-5 text-red-600 border-gray-300 rounded focus:ring-red-500">
                    <span class="ml-2 text-sm text-gray-700">Urgent</span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $newsItem->is_published) ? 'checked' : '' }}
                        class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
