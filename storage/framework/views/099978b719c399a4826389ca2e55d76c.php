    ...
    <div class="bg-white rounded-lg shadow-sm p-6 no-print">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-end gap-3">
                <form method="GET" action="<?php echo e(URL::signedRoute('admin.analytics.facility-utilization')); ?>" class="flex flex-wrap items-end gap-3">
                    <div>
                ...
                <div class="relative inline-block" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                        <a href="<?php echo e(URL::signedRoute('admin.analytics.export-facility-utilization-excel', ['start_date' => $startDate, 'end_date' => $endDate])); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-lgu-bg">
                            <i data-lucide="file-spreadsheet" class="w-4 h-4 inline mr-2"></i>
                            Export as Excel
                        </a>
                        <a href="<?php echo e(URL::signedRoute('admin.analytics.facility-utilization.export', ['start_date' => $startDate, 'end_date' => $endDate])); ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-lgu-bg">
                    ...
                    <div class="bg-gradient-to-br from-emerald-50 to-green-50 p-4 rounded-xl border border-emerald-200">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="bg-white rounded-lg p-4 border-l-4 border-red-500">
                                ...
                                <div class="flex items-baseline gap-2 mt-2">
                                    <p class="text-3xl font-bold text-red-600"><?php echo e($underutilized->count()); ?></p>
                                    <p class="text-xs text-gray-500">facilities (<30%)</p>
                                </div>
                            </div>
                        </div>
                        <div class="mt-3 p-3 bg-white rounded-lg border border-gray-200">
                            <p class="text-xs text-gray-700"><span class="font-semibold">Total Facilities:</span> <?php echo e($facilities->count()); ?></p>
                            <div class="mt-2 w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-green-500 to-red-500 rounded-full" style="width: <?php echo e($highPerforming->count() / $facilities->count() * 100); ?>%"></div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function analyzeHistoricalData() {
        ...
        const undutil = <?php echo json_encode($underutilized->count(), 15, 512) ?>;
