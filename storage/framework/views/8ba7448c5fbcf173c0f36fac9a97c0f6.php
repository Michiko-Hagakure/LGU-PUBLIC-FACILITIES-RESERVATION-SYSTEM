...
<?php $__env->startSection('page-subtitle', 'Manage your account security and privacy'); ?>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div id="content-password" class="security-tab-content p-gr-lg">
            ...
            <div class="bg-gray-50 rounded-xl p-gr-lg mb-gr-lg">
                ...
                <form action="<?php echo e(route('citizen.security.change-password')); ?>" method="POST" class="space-y-gr-md">
                    <?php echo csrf_field(); ?>
        ...
        <div id="content-sessions" class="security-tab-content hidden p-gr-lg">
            ...
            <div class="mb-gr-xl">
                ...
                <?php if($activeSessions->count() > 0): ?>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-gr-md py-gr-md">
                                        <div class="font-medium text-lgu-headline"><?php echo e($session->device_name); ?></div>
                                        <div class="text-sm text-gray-500"><?php echo e($session->ip_address); ?></div>
                                    </td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph"><?php echo e($user->full_name); ?></td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        <?php echo e($session->country ?? 'Unknown'); ?><br>
                                        <span class="text-xs text-gray-500"><?php echo e($session->city ?? 'Unknown'); ?></span>
                                    </td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        <?php echo e(\Carbon\Carbon::parse($session->logged_in_at)->format('M d, Y g:i A')); ?>
                                    ...
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        ...
                                        <?php else: ?>
                                            <?php echo e(\Carbon\Carbon::parse($session->last_active_at)->diffForHumans()); ?>
                                    ...
                                    <td class="px-gr-md py-gr-md">
                                        ...
                                        <?php else: ?>
                                            <button onclick="revokeSession('<?php echo e($session->session_id); ?>')" 
                    ...
                    <?php if($activeSessions->where('is_current', false)->count() > 0): ?>
                ...
                <?php if($trustedDevices->count() > 0): ?>
                    <div class="bg-white rounded-xl border-2 border-gray-200 overflow-hidden">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <?php $__currentLoopData = $trustedDevices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $device): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-gr-md py-gr-md font-medium text-lgu-headline"><?php echo e($device->device_name); ?></td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph"><?php echo e($device->ip_address); ?></td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        <?php echo e($device->country ?? 'Unknown'); ?><br>
                                        <span class="text-xs text-gray-500"><?php echo e($device->city ?? 'Unknown'); ?></span>
                                    </td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        <?php echo e(\Carbon\Carbon::parse($device->trusted_at)->format('M d, Y')); ?>

                                    </td>
                                    <td class="px-gr-md py-gr-md text-sm text-lgu-paragraph">
                                        <?php echo e(\Carbon\Carbon::parse($device->last_used_at)->diffForHumans()); ?>

                                    </td>
                                    <td class="px-gr-md py-gr-md">
                                        <button onclick="removeTrustedDevice(<?php echo e($device->id); ?>)" 
                    ...
                    <div class="bg-white rounded-xl border-2 border-gray-200 p-gr-md hover:shadow-md transition">
                        <div class="flex items-start justify-between">
                            <div class="flex items-start gap-gr-md flex-1">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center <?php echo e($login->status === 'success' ? 'bg-green-100' : 'bg-red-100'); ?>">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-gr-sm mb-1">
                                        <h3 class="font-semibold text-lgu-headline"><?php echo e($login->device_name); ?></h3>
                                        <span class="px-gr-sm py-0.5 rounded-full text-xs font-semibold <?php echo e($login->status === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'); ?>">
                                            <?php echo e(ucfirst($login->status)); ?>

                                        </span>
                                        <?php if($login->required_2fa): ?>
                                    ...
                                    <div class="text-sm text-lgu-paragraph space-y-0.5">
                                        <div class="flex items-center gap-gr-xs">
                                            ...
                                            <span><?php echo e($login->ip_address); ?> • <?php echo e($login->country ?? 'Unknown'); ?>, <?php echo e($login->city ?? 'Unknown'); ?></span>
                                        </div>
                                        <div class="flex items-center gap-gr-xs">
                                            <i data-lucide="calendar" class="w-3.5 h-3.5"></i>
                                            <span><?php echo e(\Carbon\Carbon::parse($login->attempted_at)->format('F d, Y \a\t g:i A')); ?></span>
                                        </div>
                                        <?php if($login->status === 'failed' && $login->failure_reason): ?>
                                            <div class="flex items-center gap-gr-xs text-red-600">
                                                <i data-lucide="alert-circle" class="w-3.5 h-3.5"></i>
                                                <span><?php echo e($login->failure_reason); ?></span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
        ...
        <div id="content-privacy" class="security-tab-content hidden p-gr-lg">
            ...
            <div class="bg-gray-50 rounded-xl p-gr-lg mb-gr-lg">
                ...
                <form action="<?php echo e(route('citizen.security.privacy')); ?>" method="POST" class="space-y-gr-md">
                    <?php echo csrf_field(); ?>
                        ...
                        <div class="space-y-gr-xs">
                            <label class="flex items-center gap-gr-sm cursor-pointer">
                                <input type="radio" name="profile_visibility" value="public" 
                                       <?php echo e($user->profile_visibility === 'public' ? 'checked' : ''); ?>
                            ...
                            <label class="flex items-center gap-gr-sm cursor-pointer">
                                <input type="radio" name="profile_visibility" value="private" 
                                       <?php echo e($user->profile_visibility === 'private' ? 'checked' : ''); ?>
                    ...
                    <div class="space-y-gr-sm">
                        <label class="flex items-center gap-gr-sm cursor-pointer">
                            <input type="checkbox" name="show_reviews_publicly" 
                                   <?php echo e($user->show_reviews_publicly ? 'checked' : ''); ?>
                        ...
                        <label class="flex items-center gap-gr-sm cursor-pointer">
                            <input type="checkbox" name="show_booking_count" 
                                   <?php echo e($user->show_booking_count ? 'checked' : ''); ?>
            ...
            <div class="bg-gray-50 rounded-xl p-gr-lg">
                ...
                <form action="<?php echo e(route('citizen.security.data-download')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="px-gr-lg py-gr-sm bg-lgu-button text-lgu-button-text font-semibold rounded-lg hover:opacity-90 transition flex items-center gap-gr-xs">
...
function enable2FAModal() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.enable-2fa")); ?>';
...
function disable2FAModal() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.disable-2fa")); ?>';
...
function revokeSession(sessionId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.revoke-session")); ?>';
...
function revokeAllSessions() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.revoke-all-sessions")); ?>';
...
function removeTrustedDevice(deviceId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.remove-device", "")); ?>/' + deviceId;
...
function removeAllDevices() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo e(route("citizen.security.remove-all-devices")); ?>';
...
<?php echo $__env->make('layouts.citizen', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\LGU-PUBLIC-FACILITIES-RESERVATION-SYSTEM\resources\views/citizen/security/index.blade.php ENDPATH**/ ?>